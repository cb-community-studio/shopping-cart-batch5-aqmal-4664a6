...
import cartsFakerFactory from "./cartsFakerFactory";
...
const CartsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = cartsFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("carts").create(elem));
...
export default connect(null, mapDispatch)(CartsFakerDialogComponent);
